...
export async function GET(_request: NextRequest) {
  if (!googleConfigured()) {
    return NextResponse.redirect(new URL("/integrations/google?error=google_not_configured", getSiteUrl()));
  }
  ...
  if (session.membership.role === "member") {
    return NextResponse.redirect(new URL("/integrations/google?error=not_allowed", getSiteUrl()));
  }
